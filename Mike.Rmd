...
Based on the `F-test`, we observed that we prefer the smaller model we got from checking the collinearity which has the predictors -- **registered**, **casual**, **temp**, **atemp** and **yr** .  From the preferred model, we decided to find the best model by removing the highest `VIF` and we ran exhaustive search to see check every possible model. We ended up using **registered** and **casual** variables to compute the predictions for each season.
...
On our preliminary assumption, Winter will be the season that needs an enhanced marketing strategy but it turned out upon finding the model that Spring is the season that would require a better marketing strategy to attract additional customers.
