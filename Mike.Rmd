...
We will explore for the best model we can use. First, we build a model based on the significant predictors that have high correlation and also build a full model. Comparing the adjusted $R^2$, both of them have high equal values which means the correlation are reliable.
...
all.equal(summary(cor_model)$adj.r.squared, summary(full_day_model)$adj.r.squared)
all.equal(summary(cor_model)$adj.r.squared, summary(full_day_model)$adj.r.squared)
