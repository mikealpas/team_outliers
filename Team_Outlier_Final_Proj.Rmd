...
bike_mod_int_poly_bic = lm(cnt ~ (. ^ 2) + I(temp ^ 2) + I(hum ^ 2) + I(windspeed ^
                                                                           2),
                            data = data_3)
# Applying BIC
...
length(coef(bike_mod_all_11))  # Total no of coef in aic model
...
filter1[is.na(filter)] <- FALSE


bike_mod_int_poly_full_bic = lm(cnt ~ (. ^ 2) + I(temp ^ 2) + I(hum ^ 2) + I(windspeed ^
                                                                           2),
                            data = data_3,
                            subset = filter1)
...
-Here, we see that by using BIC our predictors are reduced from 151 in AIC to 92. so as per the interpretation of the model, BIC will be the good fit. But when we see adjusted R2 and also LOOCV-RMSE, we see that adjusted R2 is higher and LOOCV-RMSE is lower when we select AIC.

-Since, we are fitting this model for prediction, so I will go for the model which gives lower LOOCV-RMSE and higher adj R2.
Hence, we will choose model fit via AIC.
...
full_car_model = lm(cnt ~ season + mnth + holiday + weekday + workingday + weathersit + hum + windspeed, data = day_data_converted)
```
...
without_inf_data = subset(day_data_converted, subset= cooks.distance(full_car_model) <= 4/length(cooks.distance(full_car_model)))
full_car_model = lm(cnt ~ season + mnth + holiday + weekday + workingday + weathersit + hum + windspeed, data = without_inf_data)
```
...
```{r}
#BIC process
best_bod_back_bic = step(full_car_model, direction = "backward", k = log(length(resid(full_car_model))), trace = 0)
```
...
```{r}
best_model = lm(cnt ~ season + mnth + holiday + weekday + workingday + weathersit + hum + windspeed, data = without_inf_data)
```
...
The adjusted $R^2$ indicates that the model explains `r summary(bike_mod_all_11)$r.squared` of the variance in the data.
...
So far, we have tried multiple approaches taught to us in our class to evolve our understanding of what should work to have a model with a good performance without sacrificing the high variance nature of the model. 
At this point, we have reached a decent model where we have high adjusted $R^2$ value and low LOOCV RMSE.
